...
def runSVHNDetection(imNum):
    fileName = str(imNum)
    im = cv2.imread("C:\\Users\\Jo Wood\\Documents\\GitHub\\SVHN_CNN\\CNNSampleFiles\\" + fileName +".jpg",1)
    Out = preProcessImage(im, fileName, model =3, writeIm = True)
    return Out

def preProcessImage(img, imgName, model=3, writeIm=True, cnn = None):
    ...
    contours, hierarchy  = cv2.findContours(np.uint8(pbw), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def drawBoundingBox(maybeBox,prds,img,name = 'test1', writeIm = True):
    ...
    if writeIm == True:
       cv2.imwrite(r'C:\Users\Jo Wood\Documents\GitHub\SVHN_CNN\graded_images\\' + name + '.png', oIm)
...
def imageLocalization(img):
    ...
    contours, hierarchy  = cv2.findContours(bwU, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def loadModel():
    CNNmodel = load_model(r'C:\Users\Jo Wood\Documents\GitHub\SVHN_CNN\saved_models\VGGPreTrained.classifier.hdf5')
    return CNNmodel

def loadAndDetectImages():
    for i in range(1,7,1):
        imName = str(np.uint8(i))
        Img = cv2.imread(r'C:\Users\Jo Wood\Documents\GitHub\SVHN_CNN\CNN\required\\'+ imName + '.jpg',1)
...
def createCNNVideo():
    ...
    cap = cv2.VideoCapture(r'C:\Users\Jo Wood\Documents\GitHub\SVHN_CNN\input\video_test.mp4')
    outCap = cv2.VideoWriter(r'C:\Users\Jo Wood\Documents\GitHub\SVHN_CNN\output\videoCNN.mp4', fourcc, 30.38, (1536, 864), True)
